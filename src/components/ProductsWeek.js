...
function ProductsWeek (props) {
    ...
    function addProductToCart (prod) {
        ...
        props.dispatch(props.addProduct(newCart))
    }
    function addProductToFavList (prod) {
        let newCart = []
        let cartProduct = {
            id: prod.id,
            price: prod.price,
        ...
        let duplicateProduct = props.favList.find(item=>item.id === prod.id)
        if (duplicateProduct) {
            return
        } else {
            newCart = newCart.concat(props.favList)
            newCart.push(cartProduct)
        }
        props.dispatch(props.addToFavList(newCart))
    }

    function getEmptyList () {
        let res = props.products.filter((product)=>{
        return product.label === 'week' && (props.activeCategory === "" || props.activeCategory === product.category)
        })
    ...
    return (
        <React.Fragment>
            {props.products.map((product)=>{
                return (
                    product.label === 'week' && (props.activeCategory === "" || props.activeCategory === product.category)?
                    <div className="products-list__item products-week" key={`${product.id}-week`}>
        
                        <div className="products-week__top">
                            <img src={`${process.env.PUBLIC_URL}/images/${product.img}`} alt="" className="products-week__img" />
                            <div className="products-week__nav">
                                <Link to={`/${product.id}`} className="products-week__btn">
                                ...
                                <button className="products-week__btn" onClick={()=>{
                        ...
                        <h3 className="products-week__name">
                            {product.name}
                        </h3>
                        <h4 className="products-week__price">
